...
function Navbar() {
  ...
  return (
    <div
      className={`fixed z-50 top-0 w-screen text-white ${
        expanded
          ? 'h-full bg-black flex-col'
          : 'h-[6.25rem] bg-transparent items-start'
      } flex font-semibold text-base lg:text-xl backdrop-blur-sm border-b-[1px] border-white/20 tracking-wide transition-all`}
    >
      <div className="lg:hidden sm:flex w-full flex items-center justify-between px-10 h-[6.25rem]">
        <img src="https://res.cloudinary.com/crunchbase-production/image/upload/c_lpad,h_256,w_256,f_auto,q_auto:eco,dpr_1/he3qufzpwin3r28nksdj" alt="logo" className="h-[3.1rem]" />
        <GiHamburgerMenu size={30} onClick={() => setExpanded(!expanded)} />
      </div>
      {expanded && (
        <div className="h-[calc(100vh-6.25rem)] flex flex-col text-gray-400">
          <div className="h-[13%] px-10 ">
            <Link to="/">Home</Link>
          </div>
          <div className="h-[13%] px-10 ">
          ...
          <div className="h-[15%] flex items-center justify-around text-white">
            <BsTwitter size={30} /> <BsTelegram size={30} />{' '}
            <BsLinkedin size={30} /> <BsYoutube size={30} />
          </div>
        </div>
      )}
